Это описание простого запуска своего XRAY сервера в контейнере. Есть вариант проще, через docker compose up -d, в другом репозитории
...
Для подключения, выбираем Сервер - новый профиль - VLESS и в окне заполняем:
 ...
 + Настройки сети - ставим галочку, разрешить небезопасное подключение (шифрование будет, но с сертификатом что мы выпустили в контейнере сами, а не из доверенного центра)
